...
openvpn: openssl-link lzo-link openvpn-link WISEPaaS.Datahub.Edge.so.1.0.3
...
WISEPaaS.Datahub.Edge.so.1.0.3: DatahubEdge.o cJSON.o message.o libmosquitto.so libsqlite3.so libcurl.so.4.4.0
	gcc -shared -Wl,-soname,DatahubEdge.so.1 -o DatahubEdge.so.1.0.3 DatahubEdge.o message.o cJSON.o libmosquitto.so libsqlite3.so libcurl.so.4.4.0
